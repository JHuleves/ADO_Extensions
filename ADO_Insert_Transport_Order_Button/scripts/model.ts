...
export class Model {
    ...
    constructor(buttonActions: string, buttonsNames: string, targetTypes: string, fieldsToCopy: string, targetProject: string,
        titelPrev: string, linkToWit: string, fieldsValues: string, targetFieldsList: string, buyPass: boolean, includeLinks: boolean, includeAttachments: boolean, logEnabled: boolean = false, activityInput: string = "") {
    ...
    public createSAPTask(btnIndex: number) {
        if (this.logEnabled) {
        ...
        WorkItemFormService.getService().then((service) => {
            service.getFieldValues(requiredFields).then((values) => {
                if (this.logEnabled) {
                    console.log("Parent Work Item ID: " + values["System.Id"]);
                    console.log("Read fields from parent User Story:", values);
                }
                this.executeCreateSAPTask(values, btnIndex);
            });
        });
    }

    private executeCreateSAPTask(parentFields: IDictionaryStringTo<Object>, btnIndex: number) {
        const parentId = parentFields["System.Id"].toString();
        const project = parentFields["System.TeamProject"].toString();
        const sapActivityType = parentFields["Custom.SAP_ACTIVITY_TYPE"] ? parentFields["Custom.SAP_ACTIVITY_TYPE"].toString() : "";
        const sdTicket = parentFields["Custom.ServiceDeskTicketNumber"] ? parentFields["Custom.ServiceDeskTicketNumber"].toString() : "";
        const parentTitle = parentFields["System.Title"].toString();
        const sapFaseType = parentFields["Custom.SAP_FAse_Type"] ? parentFields["Custom.SAP_FAse_Type"].toString() : "";

        const activityValue = this.activityList[btnIndex] ? this.activityList[btnIndex].trim() : "";
        let newTaskTitle = sdTicket + " - " + activityValue;
        ...
        const assignedTo = currentUser.name + " <" + currentUser.uniqueName + ">";
        ...
        this.client.createWorkItem(document, project, "Task", null, this.buyPass).then((newTask) => {
            if (this.logEnabled) {
                console.log("Task created successfully. New ID: " + newTask.id);
            }
            let linkDoc: Array<documentBuild> = [
                {
                    op: "add",
                    path: "/relations/-",
                    value: {
            ...
            // Need real parent URL for linking
            this.client.getWorkItem(+parentId).then((parentWI) => {
                linkDoc[0].value.url = parentWI.url;
                if (this.logEnabled) {
                    console.log("Linking Task " + newTask.id + " to parent " + parentId + " via:", JSON.stringify(linkDoc));
                }
                this.client.updateWorkItem(linkDoc, newTask.id).then(() => {
                    if (this.logEnabled) {
                        console.log("Linking complete.");
                    }
                    WorkItemService.WorkItemFormNavigationService.getService().then((navService) => {
